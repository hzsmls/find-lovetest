...
test('round only after averaging exact dimension values',()=>{
 ...
 // attention 6/10; attraction 5/10 (Q11 null); closeness 6/14
 assert.equal(r.total,51);assert.deepEqual(r.parts.map(p=>p.score),[60,50,43]);
});
test('ordinary social responses do not inflate closeness or attention',()=>{
 const answers=completeWith('A');
 answers[4]='A';answers[14]='B';answers[27]='B';answers[29]='D';
 const r=scoreQuiz(answers);
 assert.equal(r.parts.find(part=>part.key==='attention').score,40);
 assert.equal(r.parts.find(part=>part.key==='closeness').score,29);
});
...
test('X options are excluded and minimum coverage is enforced',()=>{
 ...
 a[5]='E';assert.equal(scoreQuiz(a).parts[0].score,null);assert.equal(scoreQuiz(a).total,null);
});
